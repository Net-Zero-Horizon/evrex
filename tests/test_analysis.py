"""
Tests for evrex analysis modules (charging, V2G, degradation, grid impact).
"""
...
class TestGenerateChargingProfiles:
    ...
    def test_peak_demand_matches_max(self, fleet, ev_params):
        result = generate_charging_profiles(fleet, ev_params, "uncontrolled")
        assert result.peak_demand_mw == pytest.approx(max(result.aggregate_hourly_mw), abs=0.01)

    def test_daily_energy_is_sum(self, fleet, ev_params):
        result = generate_charging_profiles(fleet, ev_params, "uncontrolled")
        assert result.daily_energy_mwh == pytest.approx(sum(result.aggregate_hourly_mw), abs=0.01)
    ...
    def test_tou_shifted_lower_peak(self, fleet, ev_params):
        ...
        assert tou.peak_demand_mw <= unc.peak_demand_mw * 1.1
    ...
    def test_optimized_requires_base_demand(self, fleet, ev_params, base_demand):
        result = generate_charging_profiles(
            ...
            smart_charging_fraction=0.8, base_demand_24h=base_demand,
        )
...
class TestComputeV2GPotential:
    ...
    def test_annual_potential_consistent(self, fleet, ev_params):
        v2g = compute_v2g_potential(fleet, ev_params)
        expected = v2g.daily_v2g_energy_mwh * 365 / 1000
        assert v2g.annual_v2g_potential_gwh == pytest.approx(expected, rel=0.01)

    def test_custom_connected_profile(self, fleet, ev_params):
        flat = [0.5] * 24
        v2g = compute_v2g_potential(fleet, ev_params, connected_profile=flat)
        assert v2g.hourly_connected_fraction == flat

    def test_higher_participation_more_power(self):
        fleet_light = {"light": 500}
        low = {"light": {"v2g_power": 5.0, "v2g_participation": 0.1,
                         "battery_capacity": 50.0, "efficiency_discharge": 0.9}}
        high = {"light": {"v2g_power": 5.0, "v2g_participation": 0.8,
                          "battery_capacity": 50.0, "efficiency_discharge": 0.9}}
        v_low = compute_v2g_potential(fleet_light, low)
        v_high = compute_v2g_potential(fleet_light, high)
        assert v_high.daily_v2g_energy_mwh > v_low.daily_v2g_energy_mwh

    def test_wider_soc_window_more_energy(self, fleet, ev_params):
        v_narrow = compute_v2g_potential(fleet, ev_params, v2g_min_soc=0.40, v2g_max_soc=0.60)
        v_wide = compute_v2g_potential(fleet, ev_params, v2g_min_soc=0.20, v2g_max_soc=0.90)
        assert v_wide.hourly_available_soc_mwh[0] > v_narrow.hourly_available_soc_mwh[0]

    def test_zero_fleet(self, ev_params):
        v2g = compute_v2g_potential({"light": 0}, ev_params)
        assert v2g.daily_v2g_energy_mwh == 0
...
class TestBatteryDegradation:
    ...
    def test_breakeven_is_cost_times_1000(self):
        d = compute_battery_degradation()
        assert d.breakeven_compensation == pytest.approx(d.degradation_cost_per_kwh * 1000, rel=0.01)
...
class TestAssessGridImpact:
    ...
    def ev_charging(self, fleet, ev_params):
        return generate_charging_profiles(fleet, ev_params, "uncontrolled").aggregate_hourly_mw
    ...
    def test_peak_shaving_non_negative(self, base_demand, ev_charging, v2g):
        assert assess_grid_impact(base_demand, ev_charging, v2g).peak_shaving_mw >= 0

    def test_valley_filling_non_negative(self, base_demand, ev_charging, v2g):
        assert assess_grid_impact(base_demand, ev_charging, v2g).valley_filling_mw >= 0

    def test_arbitrage_revenue_non_negative(self, base_demand, ev_charging, v2g):
        assert assess_grid_impact(base_demand, ev_charging, v2g).arbitrage_revenue_annual >= 0

    def test_v2g_dispatched_8_hours_max(self, base_demand, ev_charging, v2g):
        r = assess_grid_impact(base_demand, ev_charging, v2g)
        assert sum(1 for v in r.v2g_discharge_24h if v > 0) <= 8

    def test_custom_compensation(self, base_demand, ev_charging, v2g):
        r_low = assess_grid_impact(base_demand, ev_charging, v2g, v2g_compensation_per_mwh=10)
        r_high = assess_grid_impact(base_demand, ev_charging, v2g, v2g_compensation_per_mwh=200)
...
class TestFleetEvolutionMetrics:
    def test_returns_dict(self, ev_params):
        result = compute_fleet_evolution_metrics(
            ...
            fleet_by_category_by_year={"light": [80, 400, 800], "medium": [20, 100, 200]},
